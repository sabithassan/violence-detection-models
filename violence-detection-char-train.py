...
def print_usage():
    ...
    <n> denotes n for char -n grams. Setting n = 1 will create character unigram, setting n = 2 will create character bigram etc.
    <n> must be greater than 0
        
    example Usage:    
    python3 violence-detection-char-train.py  input/tweet-train.tsv 3
        ''')
...
def get_word_n_gram_feats (input, n):
    ...
    vectorizer = TfidfVectorizer(lowercase=False, ngram_range= n, analyzer='char',tokenizer = tokenizer)
...
if __name__ == '__main__':
    ...
    # saves trained models
    with open("models/MNB_model_char_"+str(n_gram) +"-gram.pckl", "wb") as f:
        for model in MNB_models:
            pickled.dump(model, f)

    with open("models/SVM_model_char_"+str(n_gram) +"-gram.pckl", "wb") as f:
